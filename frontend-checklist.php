	...
	public static function output($atts) {
		...
		//HTML checklist
			...
			foreach ($items as $item) {
				if ($item == '') break;
				$output .= '<p class="frontend-checklist"><label><input type="checkbox" id="frontend-checklist-'.$item['fc_listID'].'-item-'.$i.'" onchange="frontend_checklist_checkbox_changed('.$item['fc_listID'].', '.$cookie.', '.$cookie_lifetime_days.')">'.$item['text'].'</label></p>';
				$i += 1;
			}
